"""
配置模式：使用 Pydantic 定义配置结构

这个模块定义了 nanobot 的所有配置类。
...
- 前缀：NANOBOT_
- 嵌套分隔符：__
- 例如：NANOBOT_AGENTS__DEFAULTS__MODEL
"""
...
# BaseSettings：设置基类（支持环境变量）
from pydantic_settings import BaseSettings


class Base(BaseModel):
    """
    基础模型：接受 camelCase 和 snake_case 两种命名
    
    所有配置类都继承自这个基类。
    
    特性：
    - 自动转换字段名（snake_case ↔ camelCase）
    - 支持两种命名方式的输入
    """
    
    # 配置：使用 camelCase 别名，允许按名称填充
